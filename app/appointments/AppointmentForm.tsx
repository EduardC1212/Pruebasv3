...
import { saveAppointment } from '../../lib/database';
...
export default function AppointmentForm({ onSubmit, onClose }: Props) {
  ...
  const existingAppointments: Record<string, { date: string, time: string }[]> = {
    'Ana García': [
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setFormData({ person: '', date: '', time: '', reason: '' });
    onClose();
  };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2 max-h-40 overflow-y-auto">
              {availableSlots.map((slot) => (
                <button
                  ...
                  className={\`p-3 rounded-lg text-sm font-medium transition-all \${formData.time === slot
        ...
        <button
          type="submit"
          className="w-full bg-gradient-to-r from-blue-500 to-purple-500 text-white py-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200"
        >
